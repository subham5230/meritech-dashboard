...
export function QuarterlyRevenueChart({
  ...
  const {
    ...
    isLoading,
    error,
  } = trpc.companyProfiles.getQuarterlyRevenueData.useQuery({
    companyId,
  });
  ...
  const maxRevenue = Math.max(...chartData.map((d) => d.quarterlyRevenue));
  ...
  return (
    <div className="relative">
      ...
      <div className="relative w-full" style={{ height: chartHeight }}>
        ...
        >
          {/* Grid lines */}
          {[0, 100, 200, 300, 400, 500, 600, 700, 800].map((value, index) => (
            <line
              key={`grid-${index}`}
              x1={0}
          ...
          {chartData.map((point, index) => {
            ...
            const barHeight = getBarHeight(point.quarterlyRevenue, maxRevenue);
            ...
            return (
              <g key={`bar-${index}`}>
                ...
                >
                  ${point.quarterlyRevenue.toFixed(1)}M
                </text>
              </g>
            );
          })}
          ...
          <polyline
            points={chartData
              .map((point, index) => {
                ...
                const y = getLineY(point.yoyGrowth, maxGrowth);
                return `${x},${y}`;
              })
              .join(" ")}
            fill="none"
          ...
          >
            ...
            <div className="space-y-1">
              <div className="text-green-600">
                Quarterly Revenue ($M): $
                {hoveredPoint.quarterlyRevenue.toFixed(1)}M
              </div>
              <div className="text-blue-600">
                % YoY Revenue Growth: {hoveredPoint.yoyGrowth}%
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex justify-center gap-6 mt-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-700">Quarterly Revenue ($M)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-[#00ACFF] rounded-full"></div>
          <span className="text-sm text-gray-700">
            % YoY Revenue Growth (LTM)
          </span>
        </div>
      </div>
    </div>
